...
class _State extends ConsumerState<AuthorityFareScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text("Bus Fare Management"), actions: [
        IconButton(
      ...
      body: Column(children: [
            ...
            child: const Row(children: [
              ...
              Expanded(
                  child: Column(
                    ...
                    SizedBox(height: 2),
                    Text(
                        "Minimum: LKR 30  ·  Per additional stop: +LKR 8  ·  Maximum: LKR 2,422",
                        style:
                            TextStyle(color: Color(0xFF065F46), fontSize: 11)),
                  ])),
            ])),
        Container(
            color: Colors.white,
        ...
        Expanded(
          ...
          error: (e, _) => _FareEmptyState(
            icon: Icons.error_outline,
            title: "Could not load fares",
            message: e.toString().replaceFirst("Exception: ", ""),
          ),
          data: (fares) {
            final f = _search.isEmpty
                ? fares
                : fares
                    .where((r) =>
                        r.routeNumber.toLowerCase().contains(_search) ||
                        r.routeName.toLowerCase().contains(_search))
                    .toList();
            if (f.isEmpty) {
              return const _FareEmptyState(
                icon: Icons.route_outlined,
                title: "No fare data",
                message: "No routes match the current search.",
              );
            }
            return ListView.builder(
                ...
                itemBuilder: (_, i) => _FareCard(
                    fare: f[i], onEdit: () => _showEditDialog(context, f[i])));
          },
        )),
      ]),
    );
                      ...
                      .map((p) => Expanded(
                          child: Container(
                              ...
                              child: Column(children: [
                                ...
                                Text("LKR ${p.fare.toStringAsFixed(0)}",
                                    style: const TextStyle(
                                        fontWeight: FontWeight.bold,
                                        color: Color(0xFF059669),
                                        fontSize: 12)),
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Container(
                ...
                child: Column(children: [
                  ...
                  Row(
                      children: [1, 5, 10, 15, 20].map((stops) {
                    ...
                    return Expanded(
                      ...
                      Text("LKR ${fare.toStringAsFixed(0)}",
                          style: const TextStyle(
                              fontWeight: FontWeight.bold,
                              color: Color(0xFF059669),
                              fontSize: 12)),
...
class _FareEmptyState extends StatelessWidget {
  final IconData icon;
  final String title;
  final String message;
  const _FareEmptyState({
    required this.icon,
    required this.title,
  ...
  Widget build(BuildContext context) => Center(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Icon(icon, color: Colors.white.withValues(alpha: 0.35), size: 56),
            const SizedBox(height: 12),
            Text(title,
                textAlign: TextAlign.center,
                style: const TextStyle(
