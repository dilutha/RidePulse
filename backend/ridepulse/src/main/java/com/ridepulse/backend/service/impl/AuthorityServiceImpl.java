...
public class AuthorityServiceImpl implements AuthorityService {
    ...
    private FareConfigDTO toFareConfigDTO(Route route) {
        ...
        List<StopFarePreview> preview = new ArrayList<>();
        ...
        for (int stops : sampleStops) {
            if (stops > 0 && stops < totalStops) {
                ...
                preview.add(new StopFarePreview(stops, fare));
            }
        }
        // Remove duplicates
        List<StopFarePreview> finalPreview = preview;
        preview = preview.stream()
                .filter(p -> finalPreview.stream()
